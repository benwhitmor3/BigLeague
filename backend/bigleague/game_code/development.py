...
def development():
    ...
    master = players.merge(franchise, left_on='team', right_on='team')

    # adding plus one to all ages (affects entire column)
    players['age'] += 1

    # updating pv by iterating through (allows changes to be non-uniform)
    ...
    # ADD THIS IN LATER FOR PV TRAINER
    # for index, row in master.iterrows():
    #     actions_list = [row.actions1, row.actions2, row.actions3, row.actions4, row.actions5]
    #     if 'train player' in actions_list:
    #         players.at[index, 'pv'] = row['pv'] + 1
